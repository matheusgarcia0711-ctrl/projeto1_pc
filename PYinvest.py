...
locale.setlocal(locale.LC_ALL, 'pt_BR.UTF-8')
 ENTRADAS
 capital = float(input( 'Capital inicial: '))
aporte  = float(input(( 'Prazo mensal')))
meses  = float (INPUT('pRAZO (MESES): '))
cdi_anual = float (input( 'CDI anual %:  '))/100
perc_lci = float (input( 'Percentual do CDI - CDB (%): '))/100
perc_lci = float(input( 'Percentual do CDI - LCI (%): '))
